...
def create_cli(**kwargs) -> None:
    ...
    try:
        ...
        spinner.fail(
            f"Error creating Click app: {e}"
        )
        exit()


def create_api(**kwargs) -> None:
    # This function will create a core app in the current directory
    if not kwargs:
        spinner.fail("No parameters provided for creating FastAPI app.")
        exit()
    ...
    if not all(param in kwargs for param in required_params):
        spinner.fail("You must provide all parameters to create a FastAPI app.")
        exit()

    app_type="fastapi" # We're setting this manually because we already know we want the API by being within this logic block
    ...
    try:
        ...
        spinner.fail(
            f"Error creating FastAPI app: {e}"
        )
        exit()
